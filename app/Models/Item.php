...
class Item extends Model
{
    ...
    protected $fillable = [
        ...
        'has_sizes',
        'has_recipe', // Add this
        'menu_visibility', // 'both', 'resto', 'kitchen'
    ];
    ...
    // NEW: Calculate COGS using the CogsCalculator service
    public function getRecipeCostAttribute()
    {
        $calculator = new \App\Services\CogsCalculator();
        return $calculator->calculateItemCogs($this);
    }

    // NEW: Calculate COGS with specific options (portion, size, etc.)
    public function getRecipeCostWithOptionsAttribute(array $options = [])
    {
        $calculator = new \App\Services\CogsCalculator();
        return $calculator->calculateItemCogs($this, $options);
    }

    // NEW: Get profit margin based on COGS
