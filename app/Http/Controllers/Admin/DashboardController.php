...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Low stock items
        $lowStockItems = IngredientStock::whereColumn('quantity', '<=', 'min_stock')
            ->where('quantity', '>', 0)
            ->count();
        
        $outOfStockItems = IngredientStock::where('quantity', '<=', 0)
            ->count();
    ...
    {
        $sale = Sale::with(['customer', 'paymentMethod', 'user', 'saleItems.item.category', 'saleItems.kitchenItem.category'])
            ->findOrFail($id);
        
        $items = $sale->saleItems->map(function ($item) {
            $resolvedItem = $item->item ?? $item->kitchenItem;
            $categoryName = $resolvedItem && $resolvedItem->category ? $resolvedItem->category->name : null;
            return [
                'id' => $item->item_id ?? $item->kitchen_item_id,
                'name' => $item->item_name,
